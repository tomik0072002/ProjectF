...
# širší panel > je to lepší nebo horší ?! (byly by kdyžtak nutné úpravy)
...
# ── Širší sidebar + dvousloupcové rozložení uvnitř ──────────────────────────
st.markdown("""
    <style>
        /* Širší sidebar */
        ...
        /* Kompaktnější mezery mezi widgety v sidebaru */
        ...
        /* Lehce menší font popisků v sidebaru */
...
# ══════════════════════════════════════════════════════════════════════════════
#  SIDEBAR – dvousloupcové rozložení
# ══════════════════════════════════════════════════════════════════════════════
...
with st.sidebar:
    ...
    blur_kernel = st.slider(
        "Gaussovo rozostření (px)", 1, 15, 5, 2,
    ...
    if canny_auto:
        ...
        with ct1:
            canny_t1 = st.slider("Canny T1 (dolní)", 0, 254, 10)
        with ct2:
            canny_t2 = st.slider("Canny T2 (horní)", canny_t1 + 1, 255, 100)
    ...
    with flt2:
        min_circularity = st.slider(
            "Min. kruhovitost", 0.0, 1.0, 0.0, 0.05,
    ...
    with vis1:
        ...
        show_table        = st.toggle("Zobrazit tabulku",        value=True)
        show_histograms   = st.toggle("Zobrazit histogramy",     value=True)
    with vis2:
        highlight_max     = st.toggle("Zvýraznit maxima",        value=True)
        show_preprocessing = st.toggle("Předzpracování obrazu", value=False)
        show_debug        = st.toggle("Mezikroky segmentace",    value=False)
...
if show_debug:
    st.markdown("### Vizuální pipeline: Segmentace a maska")
    gray_img = cv2.cvtColor(adjusted_img, cv2.COLOR_BGR2GRAY)
    blurred = cv2.GaussianBlur(gray_img, (params["blur_kernel"], params["blur_kernel"]), 0)
